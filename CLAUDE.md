...
- **`JevClient`** (`criteria/jev.ts`, marked with `kind: 'jev'`). This judge returns a calibrated probability, which is compared against `threshold`. Its errors come back as a failed result with `error` set.
  ...
  - `openRouterJevClient` (`criteria/jev-openrouter.ts`) calls OpenRouter's **alpha** `/api/alpha/decisions` endpoint. On the wire the question type and answer field are both `noul`, and the model has a 32k-token context limit.
