...
- **`JevClient`** (`criteria/jev.ts`, marked with `kind: 'jev'`). This judge returns a calibrated probability, which is compared against `threshold`. Its errors come back as a failed result with `error` set.
  ...
  - Three clients are bundled: `openRouterJevClient` (`criteria/jev-openrouter.ts`, an **alpha** endpoint), `vercelJevClient` (`criteria/jev-vercel.ts`) and `cloudflareJevClient` (`criteria/jev-cloudflare.ts`). Each provider names the question type and answer field differently (`noul`/`noul` vs `boolean`/`probability`) and Cloudflare nests the request under `input`. Every client's wire-format caveats are recorded, with the date they were verified, in that file's header comment. Check them there before changing a client. Shared HTTP plumbing lives in `criteria/jev-http.ts`, which is internal and not exported.
